...
export const QueryPanel: React.FC<QueryPanelProps> = ({ query, setQuery, onAnalyze, isLoading }) => {
  return (
    <div className="bg-slate-800/70 backdrop-blur-md p-6 rounded-xl shadow-lg border border-slate-700 h-full flex flex-col gap-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-200 mb-2">Input Query</h2>
        <p className="text-sm text-slate-400 mb-4">Enter a radiology-related question or select a sample prompt below.</p>
        <textarea
          value={query}
      ...
      <div>
        <h3 className="text-md font-semibold text-slate-300 mb-3">Sample Prompts</h3>
        <div className="flex flex-col gap-2">
          {SAMPLE_QUESTIONS.map((q, index) => (
            <button
              ...
              onClick={() => setQuery(q)}
              disabled={isLoading}
              className="text-left text-sm text-brand-secondary hover:text-brand-light hover:bg-slate-700/50 p-2 rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {q}
            </button>
          ))}
